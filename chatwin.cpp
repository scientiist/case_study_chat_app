...
cChatWin::cChatWin() : QMainWindow(0, "", WDestructiveClose)
{
	...
	this->connect(quitButton, SIGNAL(clicked()), this, SLOT(quitButtonClosed()));
...
void cChatWin::sendButtonClicked()
{
	...
	send_string = "MSG " + std::string::basic_string(msgEdit->text().ascii()) + "\n";
...
void cChatWin::privButtonClicked()
{
	...
	send_string = "PMSG " + username + " " + std::string::basic_string(msgEdit->text().ascii()) + "\n";
	send(client_socket, send_string.c_str(), send_string.length(), 0);
	status = readLine(client_socket, buffer, MAX_LINE_BUFF, 0);
	if (status < 0) {
		theTimer->stop();
...
void cChatWin::topicButtonClicked()
{
	...
	QString topic = QInputDialog::getText("Chat Client", "Enter the new topic:", QLineEdit::Normal, QString::null, &ok, this);

	if (ok==false || topic.isEmpty()) {
		return;
	}

	send_string = "TOPIC " + std::string::basic_string(topic.ascii()) + "\n";
	send(client_socket, send_string.c_str(), send_string.length(), 0);
...
void cChatWin::timerFired()
{
	...
	// get any commands the server has sent
	while ((status = readLine(client_sockt, buffer, MAX_LINE_BUFF, 100)) != 0) {
		qApp->processEvents();
		if (status < 0) {
			QMessageBox::critical(NULL , "Lost Connection", "The server has closed the connection.");
			this->close();
